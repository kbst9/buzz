...
mod tests {
    ...
    fn fence_starts_closed_and_opens_on_advance() {
        ...
        // The fence stores unix micros: strip sub-microsecond precision
        // (present on Linux clocks, zero on macOS) so the round-trip
        // equality below tests the contract, not the platform clock.
