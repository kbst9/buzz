...
/** Instructions block with a copy button, shared by the connected-agent
 * add/edit flows (the desktop never holds a connected agent's keys, so
 * some operations ship as copy-pastable instructions for the agent's host). */
export function CopyableInstructions({
  testId,
  text,
}: {
  testId: string;
...
/**
 * Edit dialog for a CONNECTED agent — a standalone harness the desktop does
 * not manage and holds no keys for. Owner-authored instructions publish as
 * the kind:30177 definition (works offline); profile fields ride the live
 * control frame and need the agent online, with copyable host instructions
 * as the offline fallback. Opens whenever `agent` is non-null.
 */
export function ConnectedAgentEditDialog({
  agent,
  online,
  onOpenChange,
}: {
